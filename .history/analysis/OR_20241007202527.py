...
def get_odds_ratio(file_path):
    ...
    for pos, sub_df in df.groupby('site', sort=False):
        cutoff1 = sub_df['efficiency'].quantile(0.20) # 计算效率的20%分位点
        cutoff2 = sub_df['efficiency'].quantile(0.80)
        df1 = sub_df[sub_df['efficiency'] <= cutoff1].copy() # 取前20%的效率为df1
        df2 = sub_df[sub_df['efficiency'] >= cutoff2].copy() # 取后20%的效率为df2
        ...
        for i in range (1, 21):
            ...
            if i == pos:
                ...
                for base in ['A', 'C', 'G', 'T']:
                    cnt1 = df1[i].value_counts()[base] if base in base_list1 else 0 # 计算df1中base的计数
                    cnt2 = df1.shape[0] - cnt1 # 计算df1中非base的计数
                    cnt3 = df2[i].value_counts()[base] if base in base_list2 else 0 # 计算df2中base的计数
...
def OR_plot(file_path):
    ...
    # 作pos4的图像
    # x轴为不同的pos2，y轴为对应的odds值
    # for base in ['A','C','G','T']的每一个碱基，都要进行如下操作：
        # 例如，当base全为A时，分别以1-20的pos2为x轴作bar图：y轴的相对应的odds值需要进行如下比较：
        # lambda x: colors[i] if abs(x) >= np.log2(1.2) else 'gray'，如果大于于等于1.2的odds值，则用蓝色，否则用灰色
    # 遍历所有四种碱基，对pos作出4*1的图像
    ...
    plt.tight_layout()
    plt.savefig(file_path[:-4] +'_plot.png', dpi=300)
    
    
if __name__ == '__main__':
    file_path1 = './get_plots/site_Hela_ABE.csv'
    file_path2 = './get_plots/site_Hela_ABE_odds.csv'
